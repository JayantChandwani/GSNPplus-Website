...
function showStep(step) {
  document
    ...
    .forEach((stepDiv) => stepDiv.classList.remove("active"));
...
export function validateStep(step) {
  ...
  if (step === 1) {
    ...
    const family_members = document
      .getElementById("family_members")
      .value.trim();
    const hiv_positive_members = document
      .getElementById("hiv_positive_members")
      .value.trim();
    if (!family_members || !hiv_positive_members) {
      ...
      document.getElementById("error-step_2").innerText =
    ...
    const property_value = document
      .getElementById("property_value")
      .value.trim();
    if (!property_value) {
...
export function validateFileUpload() {
  ...
  const files = ["photo", "hiv_report", "address_proof", "id_proof"];
  let errorMessage = "";
